...
public partial class MealCalendar : System.Web.UI.Page
{
    ...
    protected void GetCalendarData()
    {
        ...
        m_SQL = "SELECT DISTINCT SUM(MealCount) AS MealCount, MealID, RecipeName, RecipeTypeID, SortOrder, DeliveryDate FROM vwMealDelivery WHERE DeliveryDate >= '" + firstDate.ToString("MM/dd/yyyy") + "' AND DeliveryDate < '" + lastDate.ToString("MM/dd/yyyy") + "' AND MealID IS NOT NULL AND MealTypeID = 38 AND RecipeTypeID <> 11 AND DeliveryTypeName = 'Scheduled' GROUP BY MealTypeID, MealID, DeliveryTypeName, RecipeName, RecipeTypeID, SortOrder, DeliveryDate ORDER BY MealID, SortOrder";
        calendarCellTable.CalendarScheduledDataSet = GCFDGlobals.m_GCFDPlannerDatabaseLibrary.dbSelectDataSet(m_SQL);

        m_SQL = "SELECT DISTINCT SUM(MealCount) AS MealCount, MealID, RecipeName, RecipeTypeID, SortOrder, DeliveryDate FROM vwMealDelivery WHERE DeliveryDate >= '" + firstDate.ToString("MM/dd/yyyy") + "' AND DeliveryDate < '" + lastDate.ToString("MM/dd/yyyy") + "' AND MealID IS NOT NULL AND MealTypeID = 38 AND RecipeTypeID <> 11 AND DeliveryTypeName = 'Rescheduled' GROUP BY MealTypeID, MealID, DeliveryTypeName, RecipeName, RecipeTypeID, SortOrder, DeliveryDate ORDER BY MealID, SortOrder";
        calendarCellTable.CalendarRescheduledDataSet = GCFDGlobals.m_GCFDPlannerDatabaseLibrary.dbSelectDataSet(m_SQL);

        m_SQL = "SELECT SUM(MealCount) AS MealCount, DeliveryDate FROM vwDelivery WHERE DeliveryDate >= '" + firstDate.ToString("MM/dd/yyyy") + "' AND DeliveryDate < '" + lastDate.ToString("MM/dd/yyyy") + "' AND MealTypeID = 38 AND DeliveryTypeName = 'Scheduled' GROUP BY MealTypeID, DeliveryTypeName, DeliveryDate";
        calendarCellTable.ScheduledTotalMealCountDataSet = GCFDGlobals.m_GCFDPlannerDatabaseLibrary.dbSelectDataSet(m_SQL);

        m_SQL = "SELECT SUM(MealCount) AS MealCount, DeliveryDate FROM vwDelivery WHERE DeliveryDate >= '" + firstDate.ToString("MM/dd/yyyy") + "' AND DeliveryDate < '" + lastDate.ToString("MM/dd/yyyy") + "' AND MealTypeID = 38 AND DeliveryTypeName = 'Rescheduled' GROUP BY MealTypeID, DeliveryTypeName, DeliveryDate";
    ...
    protected int TotalMealCount(string mealDate, string DeliveryTypeName)
    {
        try
        {
            ...
            m_SQL = "SELECT SUM(MealCount) AS MealCount FROM vwDelivery WHERE CONVERT(varchar(50), DeliveryDate, 101) = '" + mealDate + "' AND DeliveryTypeName = '" + DeliveryTypeName + "' AND MealTypeID = 38";
    ...
    public void MealDetailSiteNameListBoxes(string mealMode, string deliveryTypeName)
    {
        ...
        {                
            case "New":
                m_SQL = "SELECT DISTINCT DeliveryID, ParentDeliveryID, SiteName, ServingDate FROM vwMealDelivery WHERE CONVERT(varchar(10), DeliveryDate, 101) = '" + MealDateLabel.Text + "' AND DeliveryTypeName = '" + deliveryTypeName + "' AND MealID IS NULL AND MealTypeID = 38 AND DeliveryTypeName <> 'Cancelled' ORDER BY SiteName";
            ...
            case "Update":
                ...
                m_SQL = "SELECT DISTINCT DeliveryID, ParentDeliveryID, SiteName, ServingDate FROM vwMealDelivery WHERE CONVERT(varchar(10), DeliveryDate, 101) = '" + MealDateLabel.Text + "' AND DeliveryTypeName <> 'Cancelled' AND DeliveryTypeName = '" + deliveryTypeName + "' AND MealID IS NULL AND MealTypeID = 38 ORDER BY SiteName";
                mealDataSet = GCFDGlobals.m_GCFDPlannerDatabaseLibrary.dbSelectDataSet(m_SQL);
