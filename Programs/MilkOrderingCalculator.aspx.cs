...
public partial class MilkOrderingCalculator : System.Web.UI.Page
{
    ...
    protected void UpdateChocolateMilkCounts()
    {
        ...
        string m_SQL = "SELECT DISTINCT SiteName, SiteRoute, DeliveryDate, dbo.udf_DeliveryReportByRangeMealCount(DeliveryDate, SiteID,1,1) + dbo.udf_DeliveryReportByRangeMealCount(DeliveryDate, SiteID,1,2) AS MilkCrateCount FROM vwDelivery WHERE DeliveryDate >= '" + DateTextBox.Text + "' AND DeliveryDate <= '" + Date5Label.Text + "' AND MealTypeName = 'Hot' AND DeliveryTypeName <> 'Cancelled' GROUP BY DeliveryDate, SiteRoute, SiteName, SiteID ORDER BY DeliveryDate, siteroute, SiteName";
        ...
        if (m_MilkCrateDataSet.Tables[0].Rows.Count > 0)
        {
            ...
            m_SQL = "SELECT DISTINCT SiteName, SiteRoute, DeliveryDate, dbo.udf_DeliveryReportByRangeMealCount(DeliveryDate, SiteID,1,1) + dbo.udf_DeliveryReportByRangeMealCount(DeliveryDate, SiteID,1,2) AS MilkCrateCount FROM vwDelivery WHERE DeliveryDate = '" + twoWeekdaysBefore + "' OR  DeliveryDate = '" + oneWeekdayBefore + "' AND MealTypeName = 'Hot' AND DeliveryTypeName <> 'Cancelled' GROUP BY DeliveryDate, SiteRoute, SiteName, SiteID ORDER BY DeliveryDate, siteroute, SiteName";
